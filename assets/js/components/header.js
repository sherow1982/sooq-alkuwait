/**
 * مكون رأس مبسّط مع إظهار زر الهامبرجر فقط على الموبايل والتابلت
 * - اختفاء زر الهامبرجر على اللابتوب (≥ 1024px)
 */
class HeaderComponent{
  ...
  <header class="header compact-header no-desktop-nav" role="banner">
    <div class="container">
      <div class="nav-wrapper">
        ...
        <div class="header-actions compact-actions">
          ...
          <button class="mobile-menu-btn compact-menu-btn" onclick="headerComponent.toggleMobileMenu()" title="القائمة" aria-label="فتح القائمة" aria-controls="mobile-menu" aria-expanded="false">
    ...
    <div class="mobile-menu enhanced-mobile-menu" id="mobile-menu" aria-hidden="true">
      <div class="mobile-menu-header">
        ...
        <button class="mobile-menu-close" onclick="headerComponent.closeMobileMenu()" title="إغلاق" aria-label="إغلاق القائمة"><i class="fas fa-times"></i></button>
      </div>
  ...
  injectStyles(){const s=document.createElement('style');s.id='header-trim-styles';s.textContent=`
    ...
    /* إخفاء زر الهامبرجر على اللابتوب (شاشات ≥ 1024px) */
    @media(min-width:1024px){
      .mobile-menu-btn{display:none !important}
      /* ضمان عدم ظهور القائمة الجانبية على الديسكتوب */
      .enhanced-mobile-menu{right:-100% !important}
    }
    /* يظهر الزر على الموبايل والتابلت فقط (< 1024px) */
    @media(max-width:1023px){
      .mobile-menu-btn{display:flex !important}
    }
  `;const old=document.getElementById('header-trim-styles');if(old)old.remove();document.head.appendChild(s);}
  bind(){let last=window.scrollY,ticking=false;const run=()=>{const h=document.querySelector('.header');if(!h)return;const y=window.scrollY;h.classList.toggle('scrolled',y>40);if(y>last&&y>140&&!this.isMenuOpen){h.style.transform='translateY(-100%)'}else{h.style.transform='translateY(0)'}last=y;ticking=false};window.addEventListener('scroll',()=>{if(!ticking){requestAnimationFrame(run);ticking=true}}, {passive:true});document.addEventListener('click',e=>{const m=document.getElementById('mobile-menu');const b=document.querySelector('.mobile-menu-btn');if(this.isMenuOpen&&m&&!m.contains(e.target)&&(!b||!b.contains(e.target)))this.closeMobileMenu();});document.addEventListener('keydown',e=>{if(e.key==='Escape'&&this.isMenuOpen)this.closeMobileMenu();});}
  toggleMobileMenu(){const m=document.getElementById('mobile-menu');if(!m)return;this.isMenuOpen=!this.isMenuOpen;m.classList.toggle('active',this.isMenuOpen);m.setAttribute('aria-hidden',(!this.isMenuOpen).toString());document.body.style.overflow=this.isMenuOpen?'hidden':'';const btn=document.querySelector('.mobile-menu-btn');if(btn)btn.setAttribute('aria-expanded',this.isMenuOpen.toString());}
  closeMobileMenu(){const m=document.getElementById('mobile-menu');if(!m)return;this.isMenuOpen=false;m.classList.remove('active');m.setAttribute('aria-hidden','true');document.body.style.overflow='';const btn=document.querySelector('.mobile-menu-btn');if(btn)btn.setAttribute('aria-expanded','false');}
